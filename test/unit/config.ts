...
export const describeParallel: typeof describe =
    ENV_VARIABLES.NODE_ENV === 'fast' ? parallel : describe;
...
export function getStorage(storageKey: string): RxTestStorage {
    ...
    switch (storageKey) {
        ...
        case 'foundationdb':
            ...
            return {
                async init() {
                    ...
                    const { getRxStorageFoundationDB } = await nodeRequire(
                        '../../plugins/storage-foundationdb/index.cjs'
                    );
        ...
        case 'mongodb':
            ...
            return {
                async init() {
                    const { getRxStorageMongoDB } = await nodeRequire(
                        '../../plugins/storage-mongodb/index.cjs'
                    );
        ...
        case 'denokv':
            return {
                ...
                getStorage: () =>
                    wrappedValidateAjvStorage({
                        storage: getRxStorageDenoKV() as any,
                    }),
                getPerformanceStorage() {
                    return {
                        description: 'denokv',
        ...
        case 'sqlite-trial':
            ...
            return {
                ...
                async init() {
                    ...
                    initDone = true;
                    const nativeSqlitePromise = await import(
                        'node:sqlite'
            ...
            break;
        case 'sqlite-prod':
            let initDoneSqliteNew = false;
            let sqliteStorageNew: any;
            let sqliteBasicsNew;
            return {
                name: storageKey,
                async init() {
                    if (initDoneSqliteNew) {
                        return;
                    }
                    initDoneSqliteNew = true;
                ...
                getStorage() {
                    return wrappedValidateAjvStorage({
                        storage: ensureNotFalsy(sqliteStorageNew),
                    });
                },
                getPerformanceStorage() {
                    return {
                        description: 'sqlite-new-native',
                        storage: wrappedValidateAjvStorage({
                            storage: ensureNotFalsy(sqliteStorageNew),
                        }),
                    };
