...
export const RX_STORAGE_NAME_SQLITE = 'sqlite-prod';
...
export function getDatabaseConnection(
    ...
    if (!state) {
        ...
        if (
            state.sqliteBasics !== sqliteBasics &&
            databaseName !== SQLITE_IN_MEMORY_DB_NAME
        ) {
            throw new Error(
                'opened db with different creator method ' +
                    databaseName +
                    ' ' +
                    state.sqliteBasics.debugId +
                    ' ' +
                    sqliteBasics.debugId
            );
        }
...
export const TX_QUEUE_BY_DATABASE: WeakMap<
    SQLiteDatabaseClass,
    Promise<void>
> = new WeakMap();
...
export function ensureParamsCountIsCorrect(
    ...
    if (paramsCount !== paramKeyCount) {
        throw new Error(
            'ensureParamsCountIsCorrect() wrong param count: ' +
                JSON.stringify(queryWithParams)
        );
    }
}
